...
export default function Chatbot() {
  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(value => !value)} title={open ? 'Close Jioplix Assistant' : 'Open Jioplix Assistant'} className="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-primary-600 text-white shadow-[0_12px_30px_rgba(15,84,198,.28)] ring-4 ring-primary-100 transition-transform hover:-translate-y-0.5 hover:bg-primary-700" aria-label={open ? 'Close assistant' : 'Open assistant'}><MessageCircle className="h-6 w-6" /><span className="absolute -right-0.5 -top-0.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-accent-400" /></button>
    </>
  )
}
