...
export default function Landing() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7fbff] text-surface-800">
      ...
      <section className="border-y border-surface-100 bg-surface-50"><div className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-start"><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-primary-600">Built around your specialty</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-surface-900 sm:text-4xl">One platform. A more personal clinical fit.</h2><p className="mt-4 max-w-md text-[14px] leading-7 text-surface-600">Your specialty should shape the workflow, not force your team into a generic template. Start with a shared foundation and make the care experience feel like your own.</p><Link to="/login" className="mt-6 inline-flex items-center gap-2 text-[13px] font-bold text-primary-700 hover:text-primary-800">Explore specialty workflows <ArrowRight className="h-4 w-4" /></Link></div><div className="grid gap-4 sm:grid-cols-2">{specialties.map(({ icon: Icon, name, text, tone, image }) => <article key={name} className="group overflow-hidden rounded-2xl border border-surface-200 bg-white shadow-sm"><div className="relative h-36 overflow-hidden"><img src={image} alt={`${name} clinical care`} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-surface-900/55 to-transparent" /><span className="absolute bottom-3 left-4 rounded-lg bg-white/90 px-2 py-1 text-[10px] font-bold text-surface-800">{name}</span></div><div className="p-5"><div className={`inline-flex rounded-xl p-3 ${tone}`}><Icon className="h-5 w-5" /></div><p className="mt-4 text-[12px] leading-5 text-surface-500">{text}</p></div></article>)}</div></div></div></section>

      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid gap-8 overflow-hidden rounded-[2rem] bg-surface-900 p-7 text-white sm:p-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><div className="inline-flex rounded-xl bg-accent-400/15 p-3"><Sparkles className="h-6 w-6 text-accent-300" /></div><p className="mt-5 text-[11px] font-bold uppercase tracking-[.18em] text-accent-300">AI-assisted, clinician-led</p><h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">Less searching. More confident care.</h2><p className="mt-4 max-w-md text-[14px] leading-7 text-white/65">Jioplix puts practical intelligence around your team&apos;s work today, while keeping clinical judgment and prescription decisions with qualified professionals.</p></div><div className="rounded-2xl border border-white/10 bg-white/5 p-5">{aiWorkflowPoints.map(point => <div key={point} className="flex gap-3 border-b border-white/10 py-4 first:pt-0 last:border-0 last:pb-0"><Check className="mt-0.5 h-4 w-4 flex-shrink-0 text-accent-300" /><p className="text-[13px] leading-6 text-white/80">{point}</p></div>)}</div></div></section>
