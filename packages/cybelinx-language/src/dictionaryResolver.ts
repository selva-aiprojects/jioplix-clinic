import type { EffectiveTermsOptions } from './types.js'
...
export class DictionaryResolver {
  private commonDictionary: Set<string>
  private domainDictionaries: Map<string, Set<string>>
  private tenantDictionaries: Map<string, Set<string>>
  ...
  loadDomainDictionary(domain: string, words: string[]): void {
    const existing = this.domainDictionaries.get(domain.toLowerCase()) ?? new Set<string>()
    for (const w of words) {
  ...
  addTenantTerms(tenantId: string, words: string[]): void {
    ...
    const existing = this.tenantDictionaries.get(key) ?? new Set<string>()
    for (const w of words) {
      existing.add(w.toLowerCase().trim())
    }
  ...
  resolveEffectiveTerms(options?: EffectiveTermsOptions): Set<string> {
    const terms = new Set<string>(this.commonDictionary)
