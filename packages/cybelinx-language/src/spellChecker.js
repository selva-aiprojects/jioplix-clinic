...
export class SpellChecker {
    ...
    checkWord(word) {
        ...
        if (!clean || clean.length <= 1)
            return true;
        if (/^\d+(\.\d+)?(%|mg|g|kg|ml|cm|mm|bpm)?$/i.test(clean))
            return true;
        ...
        if (clean.includes('-')) {
            ...
            if (parts.every(p => !p || this.dictionary.has(p) || p.length <= 2))
                return true;
        }
        return false;
    }
     ...
     */
    getSuggestions(word, maxSuggestions = 4) {
        const clean = word.trim().toLowerCase();
        if (!clean)
            return [];
        const candidates = [];
        for (const term of this.vocabularyArray) {
            // Fast length filter
            if (Math.abs(term.length - clean.length) > 2)
                continue;
