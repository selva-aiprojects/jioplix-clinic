...
- [ ] Live SMS go-live: register DLT (MSG91/Textlocal) + set real creds in `.env` (MSG91 adapter + `test-otp-live.mjs` ready; see `docs/sms-dlt-provider.md`)
...
| Date | Summary |
|---|---|
| 2026-08-28 | **Email OTP = ₹0 production default (board: "cost-free product").** Requirement: no external registration/charges/approval delays. Honest constraint documented: an SMS to an arbitrary Indian number can never be ₹0 (TRAI DLT + per-OTP fees on every aggregator). Decision: real logins default to **email OTP** — new `EmailOtpProvider` (`apps/api/src/auth/otp/email-otp.provider.ts`): stores a hashed 6-digit code via the shared `LocalOtpStore`, resolves the recipient email **by phone** in the tenant `users` table (never from the request body), delivers a branded OTP email through the existing `MailerService`; dev (no `RESEND_API_KEY`) logs the code server-side (`[EMAIL STUB]`), prod fails closed when a user has no email. Factory tier reworked around a single **`OTP_DELIVERY`** env (default `email`): demo allowlist → `email` (₹0) | `msg91` (key+template, paid opt-in) | `supabase` (paid opt-in); a paid mode without creds degrades to email. `OtpRequestContext` gained optional `clinicName` for email copy. Demo seeder now writes deterministic emails `<name>@<slug>.demo.jioplix` (COALESCE upsert — no clobber). Tests updated: `test-otp.mjs` 26/26 (real path = email console stub, forces `OTP_DELIVERY=email`/`RESEND_API_KEY=` empty), `test-msg91.mjs` 19/19 (now boots with `OTP_DELIVERY=msg91`), `test-otp-live.mjs` now defaults to **`--email` (₹0, no secrets)** with `--msg91`/`--supabase` still `ALLOW_LIVE_SMS=true`-gated. `.env.example` documents the matrix; design doc §1–§9 rewritten (v1.1) and `docs/sms-dlt-provider.md` reframed as the paid opt-in playbook. MSG91 DLT template approval remains the only gate for paid SMS. |
| 2026-08-28 | **Phone OTP provider seam (Demo ↔ Supabase) shipped.** Implemented `docs/phone-otp-provider-design.md` as code: `apps/api/src/auth/otp/` now holds the `OtpProvider` interface + adapters on a shared `LocalOtpStore` (hashed, TTL 300s, single-use, 5-attempt bound, 60s send rate limit). `OtpProviderFactory` applies the SECURITY-CRITICAL rule — demo provider ONLY when `DEMO_OTP_ENABLED=true` AND slug ∈ {nova} AND phone ∈ `DEMO_OTP_ALLOWLIST`; every other number hard-routes to a real provider (`Msg91OtpProvider` when configured, else `SupabaseOtpProvider`). `SupabaseOtpProvider` delegates issue/verify to `supabase.auth.signInWithOtp`/`verifyOtp` when `SUPABASE_URL` + key are set; otherwise falls back to the self-contained DB OTP + SMS console stub (never client-visible). `OtpService` is now a thin port. Web `Login.tsx` demo-code UI + `SendOtpResponse.demoCode` contract (`packages/contracts`) wired. Also fixed a pre-existing boot blocker at HEAD: `EncountersModule` now imports `BillingModule` (EncountersService injects BillingService). Validated: `scripts/test-otp.mjs` rewritten to boot the API + **26/26 PASS** (hermetic after forcing `SUPABASE_URL=` empty so it can never fire live SMS) across demo/real/validation; `npm run smoke` **49/49** after relaxing DB-2 to `>=4` tenants. API build + web lint/typecheck green. |
| 2026-08-28 | **MSG91 adapter + DLT provider playbook shipped.** Added `Msg91OtpProvider` (`apps/api/src/auth/otp/msg91-otp.provider.ts`, MSG91 Auth API v5; `mobile` normalized to `91XXXXXXXXXX`; env `MSG91_AUTH_KEY`+`MSG91_TEMPLATE_ID` activates it and wins over Supabase). Factory tier now demo → MSG91 → Supabase. New `.env`/`.env.example` MSG91 vars. New `docs/sms-dlt-provider.md` (why DLT, registration checklist, provider cost/WhatsApp matrix, testing + cost control). Testing: `scripts/test-msg91.mjs` **19/19 PASS** against an inline offline MSG91 mock server (₹0 — verifies normalize/template args, OTP_INVALID, JWT session, single-use, resend-window mapping, unknown-clinic spends zero SMS, non-allowlisted hard-route); `scripts/test-otp-live.mjs` opt-in live round-trip guarded by `ALLOW_LIVE_SMS=true`. Both MSG91 & OTP scripts made re-run-proof against the shared-DB 60s rate limit. smoke 49/49, typecheck/lint green. Live Supabase/Twilio send still gated on the user enabling Phone auth in the Supabase dashboard (currently `phone=false`). |
